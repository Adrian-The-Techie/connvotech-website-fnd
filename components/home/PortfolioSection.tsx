...
function ProjectCard({ project, index, fallbackLogo }: { project: Project; index: number; fallbackLogo?: string }) {
  ...
  return (
    <motion.div
      ...
      className="group relative bg-white rounded-[40px] overflow-hidden border border-border-gray hover:border-brand-blue/20 transition-all duration-500 shadow-premium-soft hover:shadow-premium-card hover:-translate-y-2"
    >
      <div className="relative h-72 w-full overflow-hidden">
        ...
        <div className="absolute inset-0 z-30 bg-gradient-to-t from-brand-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-end p-8">
           <Link 
             ...
             className="w-full bg-primary-gradient text-white py-4 rounded-2xl flex items-center justify-center font-bold gap-2 transform translate-y-6 group-hover:translate-y-0 transition-all duration-500 shadow-premium-soft"
           >
             View Case Study
             <ArrowRight size={18} />
           </Link>
      ...
      <div className="p-8">
        ...
        <p className="text-text-gray text-sm line-clamp-2 leading-relaxed font-medium">
